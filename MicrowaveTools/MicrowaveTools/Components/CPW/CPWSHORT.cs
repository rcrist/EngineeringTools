...
namespace MicrowaveTools.Components.CPW
{
    class CPWSHORT : Comp
    {
        ...
        public CPWSHORT(double w, double S, double l, double G, int BackMetal, int Approx, Point location, int[] nodes)
        {
            Substrate subst = new Substrate();
            er = subst.er;
            h = subst.h;
        ...
        double calcLend(double frequency)
        {
        ...
        void checkProperties()
        {
            if (t >= s / 3)
            {
                Debug.WriteLine("LOG_ERROR" + "WARNING: Model for coplanar short valid for " +
                                 "t < s/3 (s/3 = %g)\n", s / 3);
            }
        }

        private Complex32 calcZ(double frequency)
        {
            double o = 2 * Math.PI * frequency;
            double l = calcLend(frequency);
            return new Complex32(0, (float)(l * o));
        }
        ...
        */
        ...
        public override void Draw(Graphics gr)
        {
            if (Orientation == "Series")
            {
                ...
                gr.DrawLine(drawPen, p7, p9);

                // Create string to draw.
                String drawString = "CPSHORT";
