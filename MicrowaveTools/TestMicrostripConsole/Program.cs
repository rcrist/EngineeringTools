...
// Local libraries
using static TestMicrostripConsole.Globals;
    ...
    class Program
    {
        ...
        class Microstrip
        {
            ...
            public void calcQuasiStatic(double W, double h, double t, double er, ref double ZlEff, ref double ErEff, ref double WEff)
            {
                ...
                if (t != 0)
                {
                    du1 = t / Math.PI * Math.Log(1 + 4 * Math.E / (t * Math.Pow(Helper.Coth(Math.Sqrt(6.517 * u)), 2.0)));
                }
                else du1 = 0;
                du = du1 * (1 + Helper.Sech(Math.Sqrt(er - 1))) / 2;
                u1 = u + du1;
                ur = u + du;
                WEff = ur * h;
        ...
        static void Main(string[] args)
        {
            ...
            double L = 1000;        // mills
            double F = 2.0;         // GHz
            double sigma = 1.0/Rho;   // S/m           

            Microstrip mlin = new Microstrip(subst, W, L, sigma);

            // Microstrip analysis
            Console.WriteLine("Microstip Analysis\n");
            mlin.calcPropagation(F);

            Console.WriteLine("\nCharacteristic Impedance");
            Console.WriteLine("fu = " + mlin.fu);
            Console.WriteLine("Zo,air = " + mlin.zr);
            Console.WriteLine("Zo = " + mlin.Zo);

            Console.WriteLine("\nCharacteristic Dielectric Constant");
            Console.WriteLine("a = " + mlin.a);
            Console.WriteLine("b = " + mlin.b);

            Console.WriteLine("\nMicrostrip Thickness Correction");
            Console.WriteLine("u1 = " + mlin.u1);
            Console.WriteLine("ur = " + mlin.ur);

            Console.WriteLine("EfEff = " + mlin.ereff);
            Console.WriteLine("Zo(Efeff) = " + mlin.ZlEff);

            Console.WriteLine("Elec Length = " + mlin.eLen);
            Console.WriteLine("Cond Loss = " + mlin.ac_db + " dB");
            Console.WriteLine("Diel Loss = " + mlin.ad_db + " dB");
            Console.WriteLine("Total Loss = " + (mlin.ac_db + mlin.ad_db));
            Console.WriteLine("Skin Depth = " + mlin.skindepth);
