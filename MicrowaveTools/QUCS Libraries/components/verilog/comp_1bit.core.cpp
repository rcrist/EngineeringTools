/*
 * comp_1bit.core.cpp - device implementations for comp_1bit module
 *
...
#include "comp_1bit.analogfunction.h"
#include "component.h"
#include "device.h"
#include "comp_1bit.core.h"

#ifndef CIR_comp_1bit
#define CIR_comp_1bit -1
#endif
...
/* Device constructor. */
comp_1bit::comp_1bit() : circuit (11)
{
  type = CIR_comp_1bit;
}

/* Initialization of model. */
void comp_1bit::initModel (void)
{
...
void comp_1bit::restartDC (void)
{
}

/* Initialize Verilog-AMS code. */
void comp_1bit::initVerilog (void)
{
...
void comp_1bit::loadVariables (void)
{
...
void comp_1bit::initializeModel (void)
{
#if defined(_DYNAMIC)
#endif
{
...
#endif
#if defined(_DERIVATE)
mE_VX_GND=(-(((1-NP(Y)))+(-1.0)*NP(Y)));
mE_VY_GND=(-((NP(X)*(-1.0))+((1-NP(X)))));
#endif
mE=(1-((NP(X)*(1-NP(Y)))+((1-NP(X))*NP(Y))));
#if defined(_DERIVATE)
mG_VX_GND=((1-NP(Y)));
mG_VY_GND=(NP(X)*(-1.0));
#endif
...
#if defined(_DERIVATE)
_load_static_jacobian1(En1,Y,((-0.5)*(TR*mE_VY_GND)*d10_tanh0));
_load_static_jacobian1(En1,X,((-0.5)*(TR*mE_VX_GND)*d10_tanh0));
#endif
}
...
#if defined(_DERIVATE)
_load_static_jacobian1(Gn1,Y,((-0.5)*(TR*mG_VY_GND)*d10_tanh0));
_load_static_jacobian1(Gn1,X,((-0.5)*(TR*mG_VX_GND)*d10_tanh0));
#endif
}
...
#if defined(_DERIVATE)
_load_static_jacobian1(Ln1,Y,((-0.5)*(TR*mL_VY_GND)*d10_tanh0));
_load_static_jacobian1(Ln1,X,((-0.5)*(TR*mL_VX_GND)*d10_tanh0));
#endif
}
...
void comp_1bit::saveOperatingPoints (void)
{
  // save global instance operating points
}

/* Load operating points. */
void comp_1bit::loadOperatingPoints (void)
{
}

/* Calculate operating points. */
void comp_1bit::calcOperatingPoints (void)
{
}

/* Initialization of AC analysis. */
void comp_1bit::initAC (void)
{
  allocMatrixMNA ();
}

/* Perform AC calculations. */
void comp_1bit::calcAC (nr_double_t frequency)
{
  setMatrixY (calcMatrixY (frequency));
}

/* Compute Y-matrix for AC analysis. */
matrix comp_1bit::calcMatrixY (nr_double_t frequency)
{
  _freq = frequency;
  saveOperatingPoints ();
  matrix y (11);
...
void comp_1bit::initSP (void)
{
  allocMatrixS ();
}

/* Perform S-parameter calculations. */
void comp_1bit::calcSP (nr_double_t frequency)
{
  setMatrixS (ytos (calcMatrixY (frequency)));   
}

/* Initialization of transient analysis. */
void comp_1bit::initTR (void)
{
  setStates (2 * 11 * 11);
  initDC ();
}
...
void comp_1bit::calcTR (nr_double_t)
{
  doHB = 0;
  doAC = 1;
  doTR = 1;
...
matrix comp_1bit::calcMatrixCy (nr_double_t frequency) 
{
  _freq = frequency;
  matrix cy (11);
