/*
 * jkff_SR.core.cpp - device implementations for jkff_SR module
 *
...
#include "jkff_SR.analogfunction.h"
#include "component.h"
#include "device.h"
#include "jkff_SR.core.h"

#ifndef CIR_jkff_SR
#define CIR_jkff_SR -1
#endif
...
/* Device constructor. */
jkff_SR::jkff_SR() : circuit (15)
{
  type = CIR_jkff_SR;
}

/* Initialization of model. */
void jkff_SR::initModel (void)
{
...
void jkff_SR::restartDC (void)
{
}

/* Initialize Verilog-AMS code. */
void jkff_SR::initVerilog (void)
{
...
void jkff_SR::loadVariables (void)
{
...
void jkff_SR::initializeModel (void)
{
#if defined(_DYNAMIC)
#endif
{
...
void jkff_SR::initializeInstance (void)
{
}

/* Evaluate Verilog-AMS equations in initial step. */
void jkff_SR::initialStep (void)
{
}

/* Evaluate Verilog-AMS equations in final step. */
void jkff_SR::finalStep (void)
{
}

/* Evaluate Verilog-AMS equations in analog block. */
void jkff_SR::calcVerilog (void)
{

/* ----------------- evaluate verilog analog equations -------------------- */
{
double m00_tanh(d00_tanh0,((TR_H*((NP(J)*NP(QB))+((1-NP(KO))*NP(QO))))-0.5))
#if defined(_DERIVATE)
double m10_tanh(d10_tanh0,d00_tanh0,((TR_H*((NP(J)*NP(QB))+((1-NP(KO))*NP(QO))))-0.5))
#endif
_load_static_residual1(Dsig,((-0.5)*d00_tanh0));
#if defined(_DERIVATE)
_load_static_jacobian1(Dsig,QO,((-0.5)*(TR_H*((1-NP(KO))))*d10_tanh0));
_load_static_jacobian1(Dsig,KO,((-0.5)*(TR_H*(-1.0)*NP(QO))*d10_tanh0));
_load_static_jacobian1(Dsig,QB,((-0.5)*(TR_H*(NP(J)))*d10_tanh0));
_load_static_jacobian1(Dsig,J,((-0.5)*(TR_H*(NP(QB)))*d10_tanh0));
#endif
}
_load_static_residual1(Dsig,NP(Dsig));
#if defined(_DERIVATE)
_load_static_jacobian1(Dsig,Dsig,1.0);
#endif
{
...
#endif
{
double m00_tanh(d00_tanh0,(TR_L*(((NP(n3A)*NP(Dsig))*NP(R))-0.5)))
#if defined(_DERIVATE)
double m10_tanh(d10_tanh0,d00_tanh0,(TR_L*(((NP(n3A)*NP(Dsig))*NP(R))-0.5)))
#endif
_load_static_residual1(n4,((-0.5)*(1-d00_tanh0)));
#if defined(_DERIVATE)
_load_static_jacobian1(n4,R,((-0.5)*(-(TR_L*((NP(n3A)*NP(Dsig))))*d10_tanh0)));
_load_static_jacobian1(n4,Dsig,((-0.5)*(-(TR_L*(NP(n3A))*NP(R))*d10_tanh0)));
_load_static_jacobian1(n4,n3A,((-0.5)*(-(TR_L*(NP(Dsig))*NP(R))*d10_tanh0)));
#endif
}
...
void jkff_SR::saveOperatingPoints (void)
{
  // save global instance operating points
}

/* Load operating points. */
void jkff_SR::loadOperatingPoints (void)
{
}

/* Calculate operating points. */
void jkff_SR::calcOperatingPoints (void)
{
}

/* Initialization of AC analysis. */
void jkff_SR::initAC (void)
{
  allocMatrixMNA ();
}

/* Perform AC calculations. */
void jkff_SR::calcAC (nr_double_t frequency)
{
  setMatrixY (calcMatrixY (frequency));
}

/* Compute Y-matrix for AC analysis. */
matrix jkff_SR::calcMatrixY (nr_double_t frequency)
{
  _freq = frequency;
  saveOperatingPoints ();
  matrix y (15);
...
void jkff_SR::initSP (void)
{
  allocMatrixS ();
}

/* Perform S-parameter calculations. */
void jkff_SR::calcSP (nr_double_t frequency)
{
  setMatrixS (ytos (calcMatrixY (frequency)));   
}

/* Initialization of transient analysis. */
void jkff_SR::initTR (void)
{
  setStates (2 * 15 * 15);
  initDC ();
}
...
void jkff_SR::calcTR (nr_double_t)
{
  doHB = 0;
  doAC = 1;
  doTR = 1;
...
matrix jkff_SR::calcMatrixCy (nr_double_t frequency) 
{
  _freq = frequency;
  matrix cy (15);
