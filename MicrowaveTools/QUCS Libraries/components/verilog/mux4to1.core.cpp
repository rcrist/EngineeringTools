/*
 * mux4to1.core.cpp - device implementations for mux4to1 module
 *
...
#include "mux4to1.analogfunction.h"
#include "component.h"
#include "device.h"
#include "mux4to1.core.h"

#ifndef CIR_mux4to1
#define CIR_mux4to1 -1
#endif
...
/* Device constructor. */
mux4to1::mux4to1() : circuit (10)
{
  type = CIR_mux4to1;
}

/* Initialization of model. */
void mux4to1::initModel (void)
{
...
void mux4to1::restartDC (void)
{
}

/* Initialize Verilog-AMS code. */
void mux4to1::initVerilog (void)
{
...
void mux4to1::loadVariables (void)
{
...
void mux4to1::initializeModel (void)
{
#if defined(_DYNAMIC)
#endif
{
...
VBI=(1-NP(B));
#if defined(_DERIVATE)
In1_VEN_GND=(-1.0)*(((((NP(D3)*NP(B))*NP(A))+((NP(D2)*NP(B))*VAI))+((NP(D1)*VBI)*NP(A)))+((NP(D0)*VBI)*VAI));
In1_VD3_GND=((1-NP(EN))*(NP(B))*NP(A));
In1_VB_GND=((1-NP(EN))*((((NP(D3))*NP(A)+(NP(D2))*VAI)+(NP(D1)*VBI_VB_GND)*NP(A))+(NP(D0)*VBI_VB_GND)*VAI));
In1_VA_GND=((1-NP(EN))*(((((NP(D3)*NP(B)))+((NP(D2)*NP(B))*VAI_VA_GND))+((NP(D1)*VBI)))+((NP(D0)*VBI)*VAI_VA_GND)));
In1_VD2_GND=((1-NP(EN))*(NP(B))*VAI);
In1_VD1_GND=((1-NP(EN))*(VBI)*NP(A));
In1_VD0_GND=((1-NP(EN))*(VBI)*VAI);
#endif
In1=((1-NP(EN))*(((((NP(D3)*NP(B))*NP(A))+((NP(D2)*NP(B))*VAI))+((NP(D1)*VBI)*NP(A)))+((NP(D0)*VBI)*VAI)));
{
...
void mux4to1::saveOperatingPoints (void)
{
  // save global instance operating points
}

/* Load operating points. */
void mux4to1::loadOperatingPoints (void)
{
}

/* Calculate operating points. */
void mux4to1::calcOperatingPoints (void)
{
}

/* Initialization of AC analysis. */
void mux4to1::initAC (void)
{
  allocMatrixMNA ();
}

/* Perform AC calculations. */
void mux4to1::calcAC (nr_double_t frequency)
{
  setMatrixY (calcMatrixY (frequency));
}

/* Compute Y-matrix for AC analysis. */
matrix mux4to1::calcMatrixY (nr_double_t frequency)
{
  _freq = frequency;
  saveOperatingPoints ();
  matrix y (10);
...
void mux4to1::initSP (void)
{
  allocMatrixS ();
}

/* Perform S-parameter calculations. */
void mux4to1::calcSP (nr_double_t frequency)
{
  setMatrixS (ytos (calcMatrixY (frequency)));   
}

/* Initialization of transient analysis. */
void mux4to1::initTR (void)
{
  setStates (2 * 10 * 10);
  initDC ();
}
...
void mux4to1::calcTR (nr_double_t)
{
  doHB = 0;
  doAC = 1;
  doTR = 1;
...
matrix mux4to1::calcMatrixCy (nr_double_t frequency) 
{
  _freq = frequency;
  matrix cy (10);
