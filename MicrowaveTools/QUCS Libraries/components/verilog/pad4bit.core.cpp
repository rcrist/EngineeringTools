/*
 * pad4bit.core.cpp - device implementations for pad4bit module
 *
...
#include "pad4bit.analogfunction.h"
#include "component.h"
#include "device.h"
#include "pad4bit.core.h"

#ifndef CIR_pad4bit
#define CIR_pad4bit -1
#endif
...
/* Device constructor. */
pad4bit::pad4bit() : circuit (4)
{
  type = CIR_pad4bit;
}

/* Initialization of model. */
void pad4bit::initModel (void)
{
...
void pad4bit::restartDC (void)
{
}

/* Initialize Verilog-AMS code. */
void pad4bit::initVerilog (void)
{
...
void pad4bit::loadVariables (void)
{
...
#endif
_load_static_residual1(D,(-ID));
#if defined(_DERIVATE)
#endif
_load_static_residual1(D,NP(D));
#if defined(_DERIVATE)
_load_static_jacobian1(D,D,1.0);
#endif
...
void pad4bit::saveOperatingPoints (void)
{
  // save global instance operating points
}

/* Load operating points. */
void pad4bit::loadOperatingPoints (void)
{
}

/* Calculate operating points. */
void pad4bit::calcOperatingPoints (void)
{
}

/* Initialization of AC analysis. */
void pad4bit::initAC (void)
{
  allocMatrixMNA ();
}

/* Perform AC calculations. */
void pad4bit::calcAC (nr_double_t frequency)
{
  setMatrixY (calcMatrixY (frequency));
}

/* Compute Y-matrix for AC analysis. */
matrix pad4bit::calcMatrixY (nr_double_t frequency)
{
  _freq = frequency;
  saveOperatingPoints ();
  matrix y (4);
...
void pad4bit::initSP (void)
{
  allocMatrixS ();
}

/* Perform S-parameter calculations. */
void pad4bit::calcSP (nr_double_t frequency)
{
  setMatrixS (ytos (calcMatrixY (frequency)));   
}

/* Initialization of transient analysis. */
void pad4bit::initTR (void)
{
  setStates (2 * 4 * 4);
  initDC ();
}
...
void pad4bit::calcTR (nr_double_t)
{
  doHB = 0;
  doAC = 1;
  doTR = 1;
...
matrix pad4bit::calcMatrixCy (nr_double_t frequency) 
{
  _freq = frequency;
  matrix cy (4);
