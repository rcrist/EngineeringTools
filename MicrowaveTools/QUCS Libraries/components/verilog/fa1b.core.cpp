/*
 * fa1b.core.cpp - device implementations for fa1b module
 *
...
void fa1b::restartDC (void)
{
}

/* Initialize Verilog-AMS code. */
void fa1b::initVerilog (void)
{
...
void fa1b::initializeModel (void)
{
#if defined(_DYNAMIC)
#endif
{
...
KS=(NP(A)*NP(B));
#if defined(_DERIVATE)
HCO_VA_GND=(((1-NP(B)))+(-1.0)*NP(B));
HCO_VB_GND=((NP(A)*(-1.0))+((1-NP(A))));
#endif
HCO=((NP(A)*(1-NP(B)))+((1-NP(A))*NP(B)));
#if defined(_DERIVATE)
ICO_VA_GND=(KS_VA_GND+(NP(CI)*HCO_VA_GND));
ICO_VB_GND=(KS_VB_GND+(NP(CI)*HCO_VB_GND));
ICO_VCI_GND=(HCO);
#endif
ICO=(KS+(NP(CI)*HCO));
#if defined(_DERIVATE)
IS_VCI_GND=(((1-HCO))+(-1.0)*HCO);
IS_VA_GND=((NP(CI)*(-HCO_VA_GND))+((1-NP(CI))*HCO_VA_GND));
IS_VB_GND=((NP(CI)*(-HCO_VB_GND))+((1-NP(CI))*HCO_VB_GND));
#endif
IS=((NP(CI)*(1-HCO))+((1-NP(CI))*HCO));
if
((ICO>=0.5))
{
#if defined(_DERIVATE)
...
_load_static_jacobian1(Sn1,CI,((-0.5)*(TR*IS_VCI_GND)*d10_tanh0));
#endif
}
...
#if defined(_DERIVATE)
_load_static_jacobian1(COn1,CI,((-0.5)*(TR*ICO_VCI_GND)*d10_tanh0));
...
void fa1b::saveOperatingPoints (void)
{
  // save global instance operating points
}

/* Load operating points. */
void fa1b::loadOperatingPoints (void)
{
}

/* Calculate operating points. */
void fa1b::calcOperatingPoints (void)
{
}

/* Initialization of AC analysis. */
void fa1b::initAC (void)
{
  allocMatrixMNA ();
}

/* Perform AC calculations. */
void fa1b::calcAC (nr_double_t frequency)
{
  setMatrixY (calcMatrixY (frequency));
}

/* Compute Y-matrix for AC analysis. */
matrix fa1b::calcMatrixY (nr_double_t frequency)
{
  _freq = frequency;
  saveOperatingPoints ();
  matrix y (9);
...
void fa1b::initSP (void)
{
  allocMatrixS ();
}

/* Perform S-parameter calculations. */
void fa1b::calcSP (nr_double_t frequency)
{
  setMatrixS (ytos (calcMatrixY (frequency)));   
}

/* Initialization of transient analysis. */
void fa1b::initTR (void)
{
  setStates (2 * 9 * 9);
  initDC ();
}
...
matrix fa1b::calcMatrixCy (nr_double_t frequency) 
{
  _freq = frequency;
  matrix cy (9);
